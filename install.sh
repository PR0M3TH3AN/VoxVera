...
# Clean generated files from existing site directories so rebuild-all
# regenerates them from the current template.  Preserves:
#   config.json          — site content and settings
#   .onionshare-session  — Tor domain identity key
#   server.pid           — running process marker
#   onionshare.log       — current log (may be in use)
# Everything else is regenerated by build_assets().
if [ -d "$HOME/host" ]; then
  for site_dir in "$HOME"/host/*/; do
    [ -f "${site_dir}config.json" ] || continue
    msg "Cleaning generated files in ${site_dir}..."
    rm -f "${site_dir}index.html" 2>/dev/null || true
    rm -f "${site_dir}qrcode-content.png" "${site_dir}qrcode-tear-offs.png" 2>/dev/null || true
    rm -rf "${site_dir}download" 2>/dev/null || true
  done
fi
