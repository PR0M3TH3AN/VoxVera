...
Features:
- **--- Создать новый сайт/листовку ---**: Start the full setup sequence.
- **--- Запустить все сайты ---**: Launch or shut down every flyer in your fleet at once.
- **Real-time Status**: View active `.onion` URLs and Tor bootstrapping progress indicators.
- **Individual Control**: Экспорт в Zip specific sites to ZIP or delete them.
...
When you host a flyer, the **"Скачать инструмент и код"** button on the landing page provides a `voxvera-portable.zip` containing:
