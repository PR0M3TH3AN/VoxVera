...
Features:
- **--- 新規サイト/フライヤー作成 ---**: Start the full setup sequence.
- **--- すべてのサイトを開始 ---**: Launch or shut down every flyer in your fleet at once.
- **Real-time Status**: View active `.onion` URLs and Tor bootstrapping progress indicators.
- **Individual Control**: Zipとしてエクスポート specific sites to ZIP or delete them.
...
When you host a flyer, the **"ツールとソースをダウンロード"** button on the landing page provides a `voxvera-portable.zip` containing:
