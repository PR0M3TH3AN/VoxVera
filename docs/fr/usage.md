...
Features:
- **--- Créer un nouveau site/flyer ---**: Start the full setup sequence.
- **--- Démarrer tous les sites ---**: Launch or shut down every flyer in your fleet at once.
- **Real-time Status**: View active `.onion` URLs and Tor bootstrapping progress indicators.
- **Individual Control**: Exporter en Zip specific sites to ZIP or delete them.
...
When you host a flyer, the **"Télécharger l'outil et le code"** button on the landing page provides a `voxvera-portable.zip` containing:
