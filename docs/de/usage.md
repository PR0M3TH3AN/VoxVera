...
Features:
- **--- Neue Website/Flyer erstellen ---**: Start the full setup sequence.
- **--- Alle Websites starten ---**: Launch or shut down every flyer in your fleet at once.
- **Real-time Status**: View active `.onion` URLs and Tor bootstrapping progress indicators.
- **Individual Control**: Als Zip exportieren specific sites to ZIP or delete them.
...
When you host a flyer, the **"Tool & Quellcode herunterladen"** button on the landing page provides a `voxvera-portable.zip` containing:
