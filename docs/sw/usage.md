...
Features:
- **--- Unda Tovuti/Kipeperushi Kipya ---**: Start the full setup sequence.
- **--- Anzisha Tovuti Zote ---**: Launch or shut down every flyer in your fleet at once.
- **Real-time Status**: View active `.onion` URLs and Tor bootstrapping progress indicators.
- **Individual Control**: Hamisha kama Zip specific sites to ZIP or delete them.
...
When you host a flyer, the **"Pakua Zana na Chanzo"** button on the landing page provides a `voxvera-portable.zip` containing:
