...
Özellikler:
- **{{t('cli.manage_create_new')}}**: Tam kurulum dizisini başlatın.
- **{{t('cli.manage_start_all')}}**: Filonuzdaki tüm ilanları tek seferde başlatın veya kapatın.
- **Gerçek Zamanlı Durum**: Aktif .onion URL'lerini ve Tor başlatma ilerleme göstergelerini görüntüleyin.
- **Bireysel Kontrol**: Belirli siteleri ZIP olarak {{t('cli.manage_action_export')}} veya silin.
...
Bir ilanı barındırdığınızda, açılış sayfasındaki **"{{t('web.download_button')}}"** düğmesi şunları içeren bir `voxvera-portable.zip` sunar:
