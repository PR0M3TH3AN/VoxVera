...
def bundle_source(dest_zip: Path):
    """Bundle the VoxVera source tree and install scripts for viral mirroring.

    We intentionally exclude pre-downloaded Tor binaries from the mirrored source
    archive. They dominate the zip size and are regenerated by the normal
    packaging/build flows, while the mirrored source bundle is meant to stay
    small enough for practical redistribution from flyer sites.
    """
    with zipfile.ZipFile(dest_zip, 'w', zipfile.ZIP_DEFLATED) as zipf:
        # 1. Include the voxvera/ source directory (excluding host, bin,
        #    bundled Tor artifacts, and pycache). We still include vendor so the
        #    source zip remains portable without re-resolving Python deps.
        for root, dirs, files in os.walk(ROOT):
            root_path = Path(root)
            if (
                "__pycache__" in root
                or "host" in root
                or "resources/bin" in root
                or root_path == ROOT / "resources" / "tor"
                or (ROOT / "resources" / "tor") in root_path.parents
            ):
                continue
            for file in files:
