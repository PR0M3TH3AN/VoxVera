(function () {
  ...
  // NIP-46 remote signer (bunker). Long timeouts: connect/sign need a human to
  // approve in the signer app. Session-only (the bunker token is never stored).
  ...
  let profiles = new Map(); // pubkey -> display name (from kind-0 metadata)

  const COLS = [
    { key: "title", label: () => t("col_title"),
      cell: (r) => `<span class="board-title">${escapeHtml(r.title || "—")}</span>`
        + `<button type="button" class="board-details-toggle" aria-expanded="false">${escapeHtml(t("details_label"))} ▾</button>`,
    ...
    { key: "author", label: () => t("col_author"),
      cell: (r) => {
        const npubSpan = `<span class="board-npub" title="${escapeHtml(r.npub)}">${escapeHtml(shortNpub(r.npub))}</span>`;
        const name = profiles.get(r.pubkey);
        return name ? `<span class="board-author-name">${escapeHtml(name)}</span><br>${npubSpan}` : npubSpan;
      },
      cmp: (a, b) => (profiles.get(a.pubkey) || a.npub || "").localeCompare(profiles.get(b.pubkey) || b.npub || "") },
  ...
  // Per-row actions, tucked behind a ⋯ menu: manage your own flyers
  // (edit / re-broadcast / delete), or block authors you didn't post.
  function actionsCell(r) {
    ...
    if (isMine) {
      ...
      const edit = naddr
        ? `<a class="board-menu-item" role="menuitem" href="${escapeHtml(editUrlForNaddr(naddr))}">${escapeHtml(t("act_edit"))}</a>`
        : "";
      const reb = `<button type="button" class="board-menu-item" role="menuitem" data-act="rebroadcast" data-id="${escapeHtml(r.id)}">${escapeHtml(t("act_rebroadcast"))}</button>`;
      const del = `<button type="button" class="board-menu-item danger" role="menuitem" data-act="delete" data-id="${escapeHtml(r.id)}">${escapeHtml(t("act_delete"))}</button>`;
      items = edit + reb + del;
    } else {
      const report = `<button type="button" class="board-menu-item" role="menuitem" data-act="report" data-id="${escapeHtml(r.id)}" data-pubkey="${escapeHtml(r.pubkey)}">${escapeHtml(t("act_report"))}</button>`;
      const block = `<button type="button" class="board-menu-item danger" role="menuitem" data-act="block" data-pubkey="${escapeHtml(r.pubkey)}">${escapeHtml(t("act_block"))}</button>`;
      items = report + block;
    }
    return `<div class="board-menu-wrap">`
      + `<button type="button" class="board-menu-btn" data-menu aria-haspopup="true" aria-expanded="false" aria-label="${escapeHtml(t("actions_label"))}">⋯</button>`
      + `<div class="board-menu" role="menu" hidden>${items}</div>`
      + `</div>`;
  }

  function closeRowMenus(except) {
    document.querySelectorAll("#board-rows .board-menu").forEach((menu) => {
      if (menu === except) return;
      menu.hidden = true;
      const btn = menu.parentElement && menu.parentElement.querySelector(".board-menu-btn");
      if (btn) btn.setAttribute("aria-expanded", "false");
    });
  }

  function toggleRowMenu(btn) {
    const menu = btn.parentElement && btn.parentElement.querySelector(".board-menu");
    if (!menu) return;
  ...
  function renderBody() {
    ...
    document.getElementById("board-rows").innerHTML = sorted
      .map((r) => `<tr>${COLS.map((c) =>
        `<td data-col="${c.key}" data-label="${escapeHtml(c.label())}">${c.cell(r)}</td>`).join("")}</tr>`)
      .join("");
  }
  ...
  function applyLang(lang) {
    ...
    setText("board-safety", t("safety_link"));
    ...
    setText("board-connect-nip46-toggle", t("connect_nip46"));
    setText("board-nip46-submit", t("connect"));
    setText("board-nip46-note", t("nip46_hint"));
    setText("board-nip46-scan", t("nip46_scan"));
    setText("board-nip46-uri-copy", t("nip46_copy"));
    ...
    // Report modal.
    setText("board-report-title", t("report_title"));
    setText("board-report-message", t("report_message"));
    setText("board-report-confirm", t("report_submit"));
    setText("board-report-cancel", t("cancel"));
    const reasonInput = document.getElementById("board-report-reason");
    if (reasonInput) reasonInput.placeholder = t("report_reason");
  ...
  function showGate() {
    ...
    const nip46Row = document.getElementById("board-nip46-row");
    if (nip46Row) nip46Row.hidden = true;
    const nip46Input = document.getElementById("board-nip46-input");
  ...
  // Method 4: NIP-46 remote signer (bunker). The vendored nostr-tools bundle has
  // no nip46 module, so this is a minimal hand-rolled client over a raw
  // WebSocket (mirrors the editor). It establishes the viewer pubkey AND keeps
  // the connection so reports/deletes can be signed remotely this session.
  let nip46Signer = null; // { conn, userPubkey } when connected via bunker
  ...
  function parseBunkerUri(uri) {
    ...
    return { remotePubkey: match[1].toLowerCase(), relays, secret: params.get("secret") || "" };
  }

  // Encrypt with NIP-44 (modern signers), accept either NIP-44 or legacy NIP-04
  // when decrypting responses.
  function nip46Codec(localSkHex, remotePubkey) {
    ...
    return {
      ...
      decrypt(ciphertext) {
        if (convKey) { try { return tools.nip44.decrypt(ciphertext, convKey); } catch (_) {} }
  ...
  function openNip46Connection({ relay, remotePubkey, localSkHex, localPubkey }) {
    ...
    function open() {
      return new Promise((resolve, reject) => {
        ...
        const openTimer = setTimeout(() => { try { ws.close(); } catch (_) {} reject(new Error("timeout")); }, NIP46_RELAY_TIMEOUT_MS);
        ws.onopen = () => {
        ...
        ws.onerror = () => {};
        ws.onclose = () => { if (!closed) rejectAll(new Error("disconnected")); };
      });
    }
    function request(method, params, timeoutMs) {
      return new Promise((resolve, reject) => {
        if (closed || !ws || ws.readyState !== 1) { reject(new Error("not connected")); return; }
        const id = Math.random().toString(36).slice(2);
        let content;
        try { content = codec.encrypt(JSON.stringify({ id, method, params })); } catch (e) { reject(e); return; }
        let signed;
        try {
          signed = tools.finalizeEvent({
            kind: NIP46_KIND, created_at: Math.floor(Date.now() / 1000),
            tags: [["p", remotePubkey]], content
          }, hexToBytes(localSkHex));
        } catch (e) { reject(e); return; }
        const timer = setTimeout(() => { pending.delete(id); reject(new Error("timeout")); }, timeoutMs || NIP46_SIGN_TIMEOUT_MS);
  ...
  let nip46Scan = null; // { ws, aborted } — the live nostrconnect (QR) listener

  function cancelNip46Scan() {
    if (nip46Scan) {
      nip46Scan.aborted = true;
      if (nip46Scan.ws) { try { nip46Scan.ws.close(); } catch (_) {} }
    }
    nip46Scan = null;
    const wrap = document.getElementById("board-nip46-qr-wrap");
    if (wrap) wrap.hidden = true;
  }

  function disconnectNip46() {
    cancelNip46Scan();
    if (nip46Signer && nip46Signer.conn) { try { nip46Signer.conn.close(); } catch (_) {} }
    nip46Signer = null;
  }

  function toggleNip46Row() {
    gateErrorKey = "";
    refreshGateError();
    const gen = document.getElementById("board-generated");
    if (gen) gen.hidden = true;
    const nsecRow = document.getElementById("board-nsec-row");
    if (nsecRow) nsecRow.hidden = true;
    const row = document.getElementById("board-nip46-row");
    if (!row) return;
    row.hidden = !row.hidden;
    if (row.hidden) { cancelNip46Scan(); return; }
    const input = document.getElementById("board-nip46-input");
    if (input) input.focus();
  }

  // Adopt a connected NIP-46 transport (shared by bunker + nostrconnect paths):
  // learn the user pubkey, keep the signer for reports/deletes, reveal the board.
  async function adoptNip46(conn) {
    ...
    disconnectNip46();
    nip46Signer = { conn, userPubkey: String(userPubkey).toLowerCase() };
    setConnecting(false);
    connectWithPubkey(userPubkey);
  }
  ...
  async function connectNip46() {
    ...
    if (!parsed) { gateErrorKey = "nip46_bad_uri"; refreshGateError(); return; }
    setConnecting(true);
    const tools = window.NostrTools;
    const skBytes = tools.generateSecretKey();
    const localSkHex = Array.from(skBytes, (b) => b.toString(16).padStart(2, "0")).join("");
    const localPubkey = tools.getPublicKey(skBytes);
    const conn = openNip46Connection({ relay: parsed.relays[0], remotePubkey: parsed.remotePubkey, localSkHex, localPubkey });
    try {
      await conn.open();
      await conn.request("connect", [parsed.remotePubkey, parsed.secret || ""], NIP46_CONNECT_TIMEOUT_MS);
      if (input) input.value = "";
      await adoptNip46(conn);
    } catch (_) {
  ...
  // and connects back, echoing our secret. We then bind a durable connection.
  function startNip46Scan() {
    cancelNip46Scan();
    gateErrorKey = "";
    refreshGateError();
    const tools = window.NostrTools;
    const skBytes = tools.generateSecretKey();
    const clientSkHex = Array.from(skBytes, (b) => b.toString(16).padStart(2, "0")).join("");
    ...
    const qr = document.getElementById("board-nip46-qr"); if (qr) qr.innerHTML = boardQrSvg(uri);
    const uriEl = document.getElementById("board-nip46-uri"); if (uriEl) uriEl.value = uri;
    const wrap = document.getElementById("board-nip46-qr-wrap"); if (wrap) wrap.hidden = false;
    const status = document.getElementById("board-nip46-scan-status"); if (status) status.textContent = t("nip46_waiting");

    const state = { ws: null, aborted: false };
    nip46Scan = state;
    const subId = "voxveranc-" + Math.random().toString(36).slice(2);
    let ws;
    try { ws = new WebSocket(relay); } catch (_) { gateErrorKey = "connect_failed"; refreshGateError(); return; }
    state.ws = ws;
    ...
    ws.onmessage = (m) => {
      ...
      conn.open()
        .then(() => adoptNip46(conn))
        .catch(() => { try { conn.close(); } catch (_) {} gateErrorKey = "connect_failed"; refreshGateError(); });
    };
    ws.onerror = () => {};
  }
  ...
  function updateFilterControls() {
    ...
    const mineChk = document.getElementById("board-mine-only");
    if (mineChk) mineChk.checked = mineOnly;
    const mineLbl = document.getElementById("board-mine-only-label");
    if (mineLbl) mineLbl.textContent = t("mine_only");
  }

  // Apply the active filters to allRows, manage the empty/filtered status, and
  ...
  function applyFilter() {
    ...
    if (mineOnly && connectedPubkey) {
      // "My flyers" overrides the trust filter — your own flyers may not be in
      // your follow graph.
      rows = visible.filter((r) => r.pubkey === connectedPubkey);
    } else {
      rows = showAll ? visible.slice() : visible.filter((r) => trustSet.has(r.pubkey));
    }
    setStatus(rows.length || (showAll && !mineOnly) ? null : "filter_empty");
    updateFilterControls();
    updateBlockedNote();
    renderBody();
  }

  // Fetch authors' kind-0 metadata so the board can show display names instead
  // of bare npubs. Names are untrusted (escaped at render) and length-capped.
  async function fetchProfiles(pubkeys) {
    const map = new Map();
    const list = Array.from(new Set(pubkeys.filter((p) => /^[0-9a-f]{64}$/i.test(p))));
    if (!list.length) return map;
    const { events } = await queryRelays(DEFAULT_RELAYS, { kinds: [0], authors: list, limit: 500 }, 5000);
    const latest = new Map();
    events.forEach((e) => {
    ...
    latest.forEach((e, pk) => {
      try {
        ...
        const name = String(meta.display_name || meta.name || "").trim().slice(0, 48);
  ...
  async function loadBoard() {
    ...
    applyFilter();
    // Enrich with author display names (kind-0); re-render when they arrive.
    fetchProfiles(allRows.map((r) => r.pubkey))
      .then((map) => { if (map.size) { profiles = map; renderBody(); } })
      .catch(() => {});
  }
  ...
  async function getBoardSigner() {
    const tools = window.NostrTools;
    // A live NIP-46 remote signer for the connected identity signs remotely.
    if (nip46Signer && nip46Signer.conn && nip46Signer.userPubkey === connectedPubkey) {
      return async (unsigned) => {
        const res = await nip46Signer.conn.request(
          "sign_event", [JSON.stringify({ ...unsigned, pubkey: connectedPubkey })], NIP46_SIGN_TIMEOUT_MS
        );
  ...
  // A report is a public, signed event referencing the flyer (e tag) and its
  // author (p tag) with a report type, so other clients and moderators can
  // aggregate it. Reporting also hides the flyer locally (pairs with Block).
  const REPORT_CATEGORIES = [
    { key: "report_cat_spam", type: "spam" },
    { key: "report_cat_illegal", type: "illegal" },
    { key: "report_cat_impersonation", type: "impersonation" },
    { key: "report_cat_other", type: "other" }
  ];
  let pendingReportRow = null;

  function populateReportCategories() {
    const sel = document.getElementById("board-report-category");
    if (!sel) return;
    const previous = sel.value;
    sel.innerHTML = REPORT_CATEGORIES
      .map((c) => `<option value="${c.type}">${escapeHtml(t(c.key))}</option>`).join("");
    if (previous) sel.value = previous;
  }

  function openReportModal(row) {
    pendingReportRow = row;
    const reason = document.getElementById("board-report-reason");
    if (reason) reason.value = "";
    const sel = document.getElementById("board-report-category");
    if (sel) sel.value = "spam";
    const modal = document.getElementById("board-report-modal");
    if (modal) modal.hidden = false;
  }
  ...
  function buildReport(row, type, reason) {
    const tags = [["p", row.pubkey]];
    if (/^[0-9a-f]{64}$/i.test(row.id)) tags.push(["e", row.id, type]);
    else tags[0].push(type);
    return { kind: 1984, created_at: nowSec(), tags, content: reason || "" };
  }

  async function confirmReport() {
    const row = pendingReportRow;
    const sel = document.getElementById("board-report-category");
    const reasonInput = document.getElementById("board-report-reason");
    const type = (sel && sel.value) || "other";
    const reason = (reasonInput && reasonInput.value || "").trim();
    closeReportModal();
    if (!row) return;
    const signer = await getBoardSigner();
    if (!signer) { setStatus("report_unavailable"); return; }
    await publishRaw(await signer(buildReport(row, type, reason)));
    // Reporting also hides the author for you (and re-renders via applyFilter).
    blockAuthor(row.pubkey);
    flashStatus("report_done");
  }
  ...
  async function init() {
    ...
    const nip46Toggle = document.getElementById("board-connect-nip46-toggle");
    if (nip46Toggle) nip46Toggle.addEventListener("click", toggleNip46Row);
    const nip46Submit = document.getElementById("board-nip46-submit");
    if (nip46Submit) nip46Submit.addEventListener("click", () => { connectNip46().catch(() => {}); });
    const nip46Input = document.getElementById("board-nip46-input");
    if (nip46Input) nip46Input.addEventListener("keydown", (e) => {
      if (e.key === "Enter") { e.preventDefault(); connectNip46().catch(() => {}); }
    });
    const nip46ScanBtn = document.getElementById("board-nip46-scan");
    if (nip46ScanBtn) nip46ScanBtn.addEventListener("click", startNip46Scan);
    const nip46CopyBtn = document.getElementById("board-nip46-uri-copy");
    if (nip46CopyBtn) nip46CopyBtn.addEventListener("click", () => {
      const u = document.getElementById("board-nip46-uri");
      if (u && u.value) navigator.clipboard.writeText(u.value).catch(() => u.select());
    });
    ...
    const mineChk = document.getElementById("board-mine-only");
    if (mineChk) mineChk.addEventListener("change", () => {
      mineOnly = mineChk.checked;
      if (allRows.length) applyFilter();
      else updateFilterControls();
    });
    ...
    if (rowsBody) rowsBody.addEventListener("click", async (event) => {
      // Mobile per-row "Details" disclosure.
      const detailsBtn = event.target.closest(".board-details-toggle");
      if (detailsBtn) {
        const row = detailsBtn.closest("tr");
        if (row) {
          const open = row.classList.toggle("expanded");
          detailsBtn.setAttribute("aria-expanded", open ? "true" : "false");
          detailsBtn.textContent = `${t("details_label")} ${open ? "▴" : "▾"}`;
        }
        return;
      }
      // ⋯ menu toggle, then the per-row actions inside it (edit links navigate).
      const menuBtn = event.target.closest("[data-menu]");
      if (menuBtn) { toggleRowMenu(menuBtn); return; }
      if (event.target.closest("[data-act]")) { handleRowAction(event); closeRowMenus(null); return; }
    ...
    const repCancel = document.getElementById("board-report-cancel");
    if (repCancel) repCancel.addEventListener("click", closeReportModal);
    const repConfirm = document.getElementById("board-report-confirm");
    if (repConfirm) repConfirm.addEventListener("click", () => { confirmReport().catch(() => {}); });
    const repOverlay = document.getElementById("board-report-modal");
    if (repOverlay) repOverlay.addEventListener("click", (e) => { if (e.target === repOverlay) closeReportModal(); });
    const unblock = document.getElementById("board-unblock-all");
    if (unblock) unblock.addEventListener("click", clearBlocked);
    document.addEventListener("keydown", (e) => { if (e.key === "Escape") { closeDeleteModal(); closeReportModal(); closeRowMenus(null); } });
    // Close any open row menu when clicking outside of one.
    document.addEventListener("click", (e) => {
      if (!e.target.closest(".board-menu-wrap")) closeRowMenus(null);
    });
